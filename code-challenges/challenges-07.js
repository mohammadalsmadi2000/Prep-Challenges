...
const objLat = (obj) => {
    let firstNameArray = [...obj.firstName];
    let lastNameArray = [...obj.lastName];
    firstNameArray[0] = firstNameArray[0].toUpperCase()
    let str1 = (firstNameArray).join("")
    lastNameArray[0] = lastNameArray[0].toUpperCase()
    let str2 = (lastNameArray).join("")

    return `my name is ${str1} ${str2} I am ${obj.age} YO, and I love ${obj.hobby}.`
};
...
const cvFormatter = (arr) => {
    let helperArray = [];
    let fullName;
    for (const item of arr) {// for-of https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/for...of
        if (item.yearsOfExperience <= 1) continue;
        else {
            if (!item.firstName) fullName = item.lastName;
            else if (!item.lastName) fullName = item.firstName;
            else fullName = item.firstName + " " + item.lastName;

            helperArray.push({ ['fullName']: fullName, ['tech']: item.tech })
        }
    }
    return helperArray;
};
...
const applicationsStatics = (arr) => {
    let python_devs = 0, javaScript_devs = 0, dotNet_devs = 0, java_devs = 0, totalApplicants = 0, rejectedApplicants = 0;
    for (const item of arr) {
        if (item.yearsOfExperience <= 1) { totalApplicants++; rejectedApplicants++ }
        else {
            switch (item.tech) {
                case 'JS': {
                    totalApplicants++;
    ...
    return ({ ['python_devs']: python_devs, ['javaScript_devs']: javaScript_devs, ['dotNet_devs']: dotNet_devs, ['java_devs']: java_devs, ['totalApplicants']: totalApplicants, ['rejectedApplicants']: rejectedApplicants })
};
// -------------------------------------------------------------------------------------------------------
